...
local 3Alaith = io.open("3Alaith", 'w')
3Alaith:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
sudo lua5.3 3Alaith.lua
done
]])
3Alaith:close()
...
screen -S ]]..Redis:get(SshId.."Info:Redis:User")..[[ ./3Alaith
done
]])
Run:close()
Redis:del(SshId.."Info:Redis:User:ID");Redis:del(SshId.."Info:Redis:User");Redis:del(SshId.."Info:Redis:Token:User");Redis:del(SshId.."Info:Redis:Token")
os.execute('chmod +x 3Alaith;chmod +x Run;./Run')
...
3Alaith = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..3Alaith)
LuaTele = luatele.set_config{api_id=16063372,api_hash='8ec29c4f93b643d2f2ebbf9d97d5e83b',session_name=3Alaith,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id)  or Redis:sismember(3Alaith.."Dev",msg.sender_id.user_id) then
ok = true
else
ok = false
end
...
JoinChannel = true
local chh = Redis:get(3Alaith.."chfalse")
if chh then
...
if tonumber(msg.sender_id.user_id) == tonumber(3Alaith) then
...
Redis:del(3Alaith..'3ddbots')
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
os.execute('cp -a ./NAM3Alaith/. ./'..folder..' && cd '..folder..' &&chmod +x * && screen -X -S '..folder:gsub("@","")..' quit && screen -d -m -S '..folder:gsub("@","")..' ./Run')
Redis:sadd(3Alaith..'3ddbots',folder)
end
end
os.execute('cp -a ./NAM3Alaith/. ./MK3Alaith')
allb = Redis:smembers(3Alaith..'3ddbots')
...
local Info_User = Redis:get(3Alaith.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(3Alaith..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'*عـزيـزي الـمـطـور فيجاتم حظره من الـصانـع بنجاح .*').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(3Alaith..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'*عـزيـزي الـمـطـور فيجاتم الـغاء حظره من الـصانـع بنجاح .*').Reply,"md",true)  
end 
end
end

if text == "• الغاء الامر •" or text == "الغاء" then
Redis:del(3Alaith..msg.sender_id.user_id.."bottoken")
Redis:del(3Alaith..msg.sender_id.user_id.."botuser")
Redis:del(3Alaith..msg.sender_id.user_id.."make:bot")
Redis:del(3Alaith..msg.sender_id.user_id.."brodcast")
Redis:del(3Alaith.."all:texting:pv")
Redis:del(3Alaith.."all:texting")
Redis:del(3Alaith.."KN:OPAll")
Redis:del(3Alaith.."all:BanAll")
Redis:del(3Alaith..msg.sender_id.user_id.."brodcast:fwd")
...
send(msg.chat_id,msg.id,"*◍ اهلاً بك عـزيـري الـمـطور فيجا* \n","md",true, false, false, true, reply_markup)
return false 
end
...
local list = Redis:smembers(3Alaith..'3ddbots')
send(msg.chat_id,msg.id,"تم تشغيل "..#list.." بوت","html",true)  
end

-----By BaNdA
if text == "🔎 | احصائيات المصنوعات" or text == "فحص" then
Redis:del(3Alaith.."All:pv:st")
Redis:del(3Alaith.."All:gp:st")
...
Redis:incrby(3Alaith.."All:gp:st",tonumber(#list))
Redis:incrby(3Alaith.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n◍ احصائيات جميع البوتات المصنوعه \n ◍ عدد المجموعات '..Redis:get(3Alaith.."All:gp:st")..' مجموعه \n◍ عدد المشتركين '..Redis:get(3Alaith.."All:pv:st")..' مشترك',"md",true)
end
----By BaNdA
if text == "🔬 | تنظيف الوهمي" or text == "حذف الوهمي" then
Redis:del(3Alaith.."fake")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
lt = Redis:smembers(bot_id.."Num:User:Pv") 
if #list < 2 then
Redis:sadd(3Alaith.."fake",botuser )
os.execute("sudo rm -fr "..botuser)
os.execute('screen -X -S '..botuser:gsub("@","")..' quit')
end
end
end
local list = Redis:smembers(3Alaith..'fake')
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(3Alaith..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(3Alaith..'3ddbots')
if #list <= 100 then
...
------By BaNdA
-----تشغيل البوتات ---
if text and Redis:get(3Alaith..msg.sender_id.user_id.."run:bot") then
Redis:del(3Alaith..msg.sender_id.user_id.."run:bot")
Redis:del(3Alaith.."screen:on")
Redis:del(3Alaith.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
Redis:sadd(3Alaith.."bots:folder",folder:gsub("@",""))
end
end
if not Redis:sismember(3Alaith.."bots:folder",userbot) then
...
Redis:sadd(3Alaith.."screen:on",screen)
end
local list = Redis:smembers(3Alaith..'screen:on')
for k,v in pairs(list) do
...
Redis:set(3Alaith..msg.sender_id.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"*◍ حسناً، قم بإرسال معرف البوت ليتم تشغيله*", "md")
return false 
end
---ايقاف البوتات
if text and Redis:get(3Alaith..msg.sender_id.user_id.."stop:bot") then
Redis:del(3Alaith..msg.sender_id.user_id.."stop:bot")
Redis:del(3Alaith.."screen:on")
Redis:del(3Alaith.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(3Alaith..msg.sender_id.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"*◍ قم بإرسال معرف البوت ليتم ايقافه*", "md")
return false 
end
----------
--الاشتراك الاجباري 
if Redis:get(3Alaith.."ch:addd"..msg.sender_id.user_id) == "on" then
Redis:set(3Alaith.."ch:addd"..msg.sender_id.user_id,"off")
...
Redis:del(3Alaith.."chfalse")
Redis:set(3Alaith.."chfalse",ch)
Redis:del(3Alaith.."ch:sup")
Redis:set(3Alaith.."ch:sup",da.result.invite_link)
else
...
Redis:set(3Alaith.."ch:addd"..msg.sender_id.user_id,"on")
...
if text and Redis:get(3Alaith..msg.sender_id.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(3Alaith..msg.sender_id.user_id.."bottoken")
local botuser = Redis:get(3Alaith..msg.sender_id.user_id.."botuser")
local uu = LuaTele.getUser(UserId_Info.id)
local Informationlua = io.open("./MK3Alaith/Information.lua", 'w')
Informationlua:write([[
return {
Token = "]]..bottoken..[[",
UserBot = "]]..botuser..[[",
...
Informationlua:close()
os.execute('cp -a ./MK3Alaith/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:sadd(3Alaith.."userbots",botuser)
Redis:del(3Alaith..msg.sender_id.user_id.."bottoken")
Redis:del(3Alaith..msg.sender_id.user_id.."botuser")
Redis:del(3Alaith..msg.sender_id.user_id.."make:bot")
...
if text and Redis:get(3Alaith..msg.sender_id.user_id.."make:bot") == "token" then
...
if Redis:sismember(3Alaith.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n*◍ هذا البوت بالفعل مصنوع !!*","md",true)  
return false 
end 
Redis:set(3Alaith..msg.sender_id.user_id.."botuser",botuser)
Redis:set(3Alaith..msg.sender_id.user_id.."bottoken",text)
Redis:set(3Alaith..msg.sender_id.user_id.."make:bot","devuser")
...
Redis:set(3Alaith..msg.sender_id.user_id.."make:bot","token")
...
if text and Redis:get(3Alaith..msg.sender_id.user_id.."make:bot") == "del" then
...
Redis:srem(3Alaith.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n◍ تم حذف البوت من الصانع بنجاح ..","md",true)  
return false 
end 
if text == "❎ | حذف بوت" then
Redis:set(3Alaith..msg.sender_id.user_id.."make:bot","del")
...
if tonumber(3Alaith) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'◍ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(3Alaith..'total',v)  
end
end
end
if text == "🧲 | جلب نسخه" then
local UsersBot = Redis:smembers(3Alaith.."total")
local Get_Json = '{"BotId": '..3Alaith..','  
if #UsersBot ~= 0 then 
...
if Redis:get(3Alaith.."all:texting") then
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(3Alaith.."all:texting")
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(3Alaith.."3z:gp",text)
Redis:del(3Alaith.."all:texting")
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape("["..Redis:get(3Alaith.."3z:gp").."]").."&parse_mode=Markdown")
end
end
end
Redis:del(3Alaith.."3z:gp")
Redis:del(3Alaith.."all:texting")
send(msg.chat_id,msg.id,"◍ تم انتهاء الاذاعه في كل البوتات","md",true)
end
if Redis:get(3Alaith.."all:texting:pv") then
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(3Alaith.."all:texting:pv")
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(3Alaith.."eza3a:pv",text)
Redis:del(3Alaith.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape("["..Redis:get(3Alaith.."eza3a:pv").."]").."&parse_mode=Markdown")
end
end
end
Redis:del(3Alaith.."eza3a:pv")
Redis:del(3Alaith.."all:texting:pv")
send(msg.chat_id,msg.id,"◍ تم انتهاء الاذاعه في كل البوتات","md",true)
end
if Redis:get(3Alaith.."all:texting:guu") then
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(3Alaith.."all:texting:pv")
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(3Alaith.."eza3a:guu",text)
Redis:del(3Alaith.."all:texting:pv")
send(msg.chat_id,msg.id,"*جاري عمل اذاعة لجميع المصنوعات ..*","md",true)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape("["..Redis:get(3Alaith.."eza3a:guu").."]").."&parse_mode=Markdown")
Redis:set(3Alaith.."PinMsegees:"..v,text)
end
end
end
Redis:del(3Alaith.."eza3a:guu")
Redis:del(3Alaith.."all:texting:guu")
send(msg.chat_id,msg.id,"◍ تم انتهاء الاذاعه في كل البوتات","md",true)
end
if text == "📢 | إذاعه عام للمجموعات" then
Redis:set(3Alaith.."all:texting",true)
send(msg.chat_id,msg.id,"*قم بإرسال لي النص الآن ليتم اذاعته في المجموعات ..*","md",true)
end
if text == "📮 | إذاعه عام للمشتركين" then
Redis:set(3Alaith.."all:texting:pv",true)
send(msg.chat_id,msg.id,"*قم بإرسال لي النص الآن ليتم اذاعته للمشتركين ..*","md",true)
end
if text == "📀 | اذاعه عام تثبيت" then
Redis:set(3Alaith.."all:texting:guu",true)
send(msg.chat_id,msg.id,"*قم بإرسال لي النص الآن ليتم اذاعته  بالتثبيت ..*","md",true)
end

--By BaNdA
if Redis:get(3Alaith..msg.sender_id.user_id.."brodcast") then 
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(3Alaith..msg.sender_id.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
...
if text == "📡 | اذاعه" then
Redis:set(3Alaith..msg.sender_id.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"ارسل لي النص الآن ليتم اذاعته ..")
return false 
end
---fwd
if Redis:get(3Alaith..msg.sender_id.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(3Alaith..msg.sender_id.user_id.."brodcast:fwd")
...
Redis:set(3Alaith..msg.sender_id.user_id.."brodcast:fwd",true)
send(msg.chat_id,msg.id,"قم بإرسال التوجية الآن ليتم نشره ..")
return false 
end
--By BaNdA
------By BaNdA
if Redis:get(3Alaith.."all:BanAll") then
if not Redis:get(3Alaith.."all:BanAll") then
return false 
end
...
if Redis:get(3Alaith.."KN:OPAll") then
if not Redis:get(3Alaith.."KN:OPAll") then
return false 
end
if text and text:match("^(%d+)$") then
...
local chinfo = Redis:get(3Alaith.."ch:sup")
...
if not Redis:get(3Alaith.."twsl") then
if msg.sender_id.user_id ~= tonumber(3Alaith) then
if Redis:sismember(3Alaith..'BaN:In:Tuasl',msg.sender_id.user_id) then
return false 
end
if msg.id then
Redis:setex(3Alaith.."Twasl:UserId"..msg.date,172800,msg.sender_id.user_id)
...
if text and Redis:get(3Alaith.."free:bot") then
...
{
{text = '🔘 | تواصل معنا', url = 't.me/lllEll0'}, 
},
}
}
...
if text == "/start" then
if not Redis:sismember(3Alaith.."total",msg.sender_id.user_id) then
Redis:sadd(3Alaith.."total",msg.sender_id.user_id)
local list = Redis:smembers(3Alaith.."total")
...
---making user
if text and Redis:get(3Alaith..msg.sender_id.user_id.."make:bot") then
if text == "• إلغاء •" then
Redis:del(3Alaith..msg.sender_id.user_id.."make:bot")
...
local allb = Redis:smembers(3Alaith..'3ddbots')
local Informationlua = io.open("./MK3Alaith/Information.lua", 'w')
Informationlua:write([[
return {
Token = "]]..text..[[",
...
Informationlua:close()
os.execute('cp -a ./MK3Alaith/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(3Alaith..msg.sender_id.user_id.."my:bot",botuser)
Redis:sadd(3Alaith.."userbots",botuser)
Redis:del(3Alaith..msg.sender_id.user_id.."make:bot")
...
{
{text = '☢️ | اضغط هنا لتنصيب', url = 't.me/lllEll1'}, 
},
{
{text = '🔘 | تواصل معنا', url = 't.me/lllEll0'}, 
},
...
if text == "✅ | صنع بوت" then
if Redis:get(3Alaith..msg.sender_id.user_id.."my:bot") then
...
Redis:set(3Alaith..msg.sender_id.user_id.."make:bot",true)
send(msg.chat_id,msg.id,"*◍ قم بإرسال توكن بوتك الآن ..*", 'md')
return false 
end
if text == "🔹 | طريقه عمل بوت" then
Redis:del(3Alaith..msg.sender_id.user_id.."make:bot",true)
local reply_markup = LuaTele.replyMarkup{
type = 'inline',
data = {
{
...
{{text = '- اخفاء الامر ', data =msg.sender_id.user_id..'/delAmr'},},}}
 send(msg.chat_id,msg.id,"https://telegra.ph/file/abc58c231eea6199b56e1.jpg","html",false, false, false, false, reply_markup)
end

----end making user
if text == "❎ | حذف بوت" then
if Redis:get(3Alaith..msg.sender_id.user_id.."my:bot") then
local botuser = Redis:get(3Alaith..msg.sender_id.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(3Alaith.."userbots",botuser)
Redis:del(3Alaith..msg.sender_id.user_id.."my:bot")
...
if tonumber(data.message.sender_user_id) == tonumber(3Alaith) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender_id.user_id == 3Alaith then
return false
end
